...
public class Menu implements Item {
	private static final int STARS_AMOUNT = 5;
	private String name;
	...
	private void displayTitle(InputOutput io) {
		io.writeLine("*".repeat(STARS_AMOUNT * 2 + name.length()));
		io.writeLine(String.format("*%s%s%s*", " ".repeat(STARS_AMOUNT -1), name, " ".repeat(STARS_AMOUNT -1)));
		io.writeLine("*".repeat(STARS_AMOUNT * 2 + name.length()));
